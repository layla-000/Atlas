...
function setupAtlasScheduleDateTimeRules(container) {
  ...
  [startInput, endInput].filter(Boolean).forEach((input) => {
    input.step = "300";
    installAtlasFiveMinutePicker(input, () => syncAtlasScheduleEndDate(startInput, endInput));
  });

  syncAtlasScheduleEndDate(startInput, endInput);
}

function installAtlasFiveMinutePicker(input, onChange) {
  ...
  const syncVisibleFromValue = () => {
    const value = normalizeAtlasFiveMinuteValue(input.value);
    input.value = value;
    if (!value) {
      dateInput.value = "";
      hourSelect.value = "00";
