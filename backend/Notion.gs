...
function syncKnowledgeRelationshipsToNotion(record, relationships) {
  const config = getNotionConfig_();

  if (!config.enabled || !config.relationshipDatabaseId) {
    return {
      relationshipNotionStatus: "skipped",
      message: "Notion Relationship DB 설정이 없어 저장을 건너뛰었어요."
    };
  }
  ...
  if (!relationships || !relationships.relationships || relationships.relationships.length === 0) {
    return {
      relationshipNotionStatus: "skipped",
      message: "동기화할 Relationship이 없어요."
    };
  }

  const results = relationships.relationships.map(function(relationship) {
    return createNotionRelationshipFromParser_(config, record, relationship);
  });

  return {
    relationshipNotionStatus: "completed",
    syncedAt: new Date().toISOString(),
...
function createNotionRelationshipFromParser_(config, record, relationship) {
  const tripPageId = findTripPageIdByTripId_(config, record.tripId);
  const sourceObjectPageId = findKnowledgeObjectPageIdByObjectId_(config, relationship.fromId);
  const targetObjectPageId = findKnowledgeObjectPageIdByObjectId_(config, relationship.toId);

  const relationshipType = mapRelationshipTypeForNotion_(relationship.type);
  const evidenceNote = buildRelationshipEvidenceNote_(relationship);

  const payload = {
    parent: { database_id: config.relationshipDatabaseId },
    properties: {
      "Name": {
        title: [{ text: { content: relationship.type || "relationship" } }]
      },
      "Relationship ID": {
        rich_text: [{ text: { content: relationship.id || "" } }]
      },
      "Relationship Type": {
        select: { name: relationshipType }
      },
      "Source Object": sourceObjectPageId
        ? { relation: [{ id: sourceObjectPageId }] }
        : { relation: [] },
      "Target Object": targetObjectPageId
        ? { relation: [{ id: targetObjectPageId }] }
        : { relation: [] },
      "Trip": tripPageId
        ? { relation: [{ id: tripPageId }] }
        : { relation: [] },
      "Evidence Note": {
        rich_text: evidenceNote
          ? [{ text: { content: evidenceNote } }]
          : []
      },
      "Confidence": {
        number: relationship.confidence || 0.8
      },
      "Status": {
        select: { name: "Likely" }
      },
      "Strength": {
        select: { name: mapRelationshipStrengthForNotion_(relationship.confidence) }
      },
      "Direction": {
        select: { name: "one_way" }
      },
...
function mapRelationshipTypeForNotion_(relationshipType) {
  const map = {
    "document_evidences_object": "derived_from_document"
  };

  return map[relationshipType] || "unknown";
}

function mapRelationshipStrengthForNotion_(confidence) {
...
function buildRelationshipEvidenceNote_(relationship) {
  const props = relationship && relationship.properties ? relationship.properties : {};
  const parts = [];

  if (props.sourceDocumentTitle) {
    parts.push("Source document: " + props.sourceDocumentTitle);
  } else if (props.sourceFileName) {
    parts.push("Source file: " + props.sourceFileName);
  }

  if (props.source) {
    parts.push("Generated by: " + props.source);
  }

  return parts.join(" | ").slice(0, 1800);
}

function findTripPageIdByTripId_(config, tripId) {
  if (!tripId) return null;

  const tripDbId = "35f24d7edc564020970a9691e0d021f6";
  const response = UrlFetchApp.fetch("https://api.notion.com/v1/databases/" + tripDbId + "/query", {
    method: "post",
    contentType: "application/json",
    headers: {
  ...
  if (code < 200 || code >= 300) {
    throw new Error("Notion Trip query error: " + code + " " + response.getContentText());
  }
...
function findKnowledgeObjectPageIdByObjectId_(config, objectId) {
  if (!objectId || !config.entityDatabaseId) return null;

  const response = UrlFetchApp.fetch("https://api.notion.com/v1/databases/" + config.entityDatabaseId + "/query", {
    method: "post",
    contentType: "application/json",
    headers: {
      Authorization: "Bearer " + config.token,
  ...
  if (code < 200 || code >= 300) {
    throw new Error("Notion Knowledge Object query error: " + code + " " + response.getContentText());
  }
